...
9) How many cars are in the dataset? How many variables are recorded for each car? Show the code you'd use to figure this out.

```{r q5}
dim(cars) # cars = the number of rows and variables = the number of columns
nrow(cars) # There are `nrow(cars)` observations in the dataset.
```
